var Gateway = (function() {
	...
	var CHAT_TOPIC = "chat_topic2";
	var CHAT_USER_PREFIX = "user_";

	// Application constants
	var SANDBOX_ID = "nKkG23KJnb";
	var SANDBOX_EMAIL = "peter.moskovits@kaazing.com";
	var SANDBOX_TOKEN = "e5026e8c-17d2-475d-921c-63578d0a2807";
	...
	// Application constants
	var PARSE_APP_ID = "y03NY2ZtUcPuXSKrP5nxVfzU50T7pAKC2mv6XXCu";
	var PARSE_JS_KEY = "8Udu9b8HhOhwl86ZPVsqKKGaWc1ytHI7ung8ONRC";
	...
	/*
	 * Dependencies Run as soon as the script is loaded
	 */

	var script_dependencies = [ //
	"kaazing/AmqpClient.js",//
	"kaazing/WebSocket.js",//
	"kaazing/parse-1.3.3.min.js" //
	];
	...
	function doClientOpen() {
		...
		amqp_queue = AMQP_QUEUE_PREFIX + account_information
				+ AMQP_QUEUE_SUFFIX + Date.now();
	...
	function doChatSend(event) {
		...
		// if (event.keyCode == CHAT_ENTER
		// && chat_message.innerHTML.trim().length > 0) {
		// json = JSON.stringify({
		// color : chat_color,
	...
	// }
	function chatCb(client_id, recReady, recMsg) {
		callback.onReady = recReady;
		callback.onMessage = recMsg;
	...
	function on(event, callback) {
		...
		if (event_listeners == null) {
			...
			for (var e = 0; e < event_listeners.length; e++) {
				if (event_listeners[e].event == event
						&& event_listeners[e].callback == callback) {
					found = true;
					break;
				}
			}
	...
	return {
		...
		CHAT_USER_PREFIX : CHAT_USER_PREFIX,
